...
class _StatisticsScreenState extends State<StatisticsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final hasTyt = appState.selectedExamIds.contains('tyt_2027');
    final hasAyt = appState.selectedExamIds.contains('ayt_2027');
    final hasYdt = appState.selectedExamIds.contains('ydt_2027');
    final hasYds = appState.selectedExamIds.any((id) => id.startsWith('yds_'));
    final hasKpssLisans = appState.selectedExamIds.any((id) => id.contains('kpss_lisans'));
    final hasEkpss = appState.selectedExamIds.any((id) => id.contains('ekpss'));
    final hasDgs = appState.selectedExamIds.any((id) => id.contains('dgs'));
    final hasAles = appState.selectedExamIds.any((id) => id.contains('ales'));
    final hasTus = appState.selectedExamIds.any((id) => id.contains('tus'));
    
    final availableTypes = <String>[];
    if (hasTyt) availableTypes.add('tyt');
    if (hasAyt) availableTypes.add('ayt');
    if (hasYdt) availableTypes.add('ydt');
    if (hasYds) availableTypes.add('yds');
    if (hasKpssLisans) availableTypes.add('kpss_lisans');
    if (hasEkpss) availableTypes.add('ekpss');
    if (hasDgs) availableTypes.add('dgs');
    if (hasAles) availableTypes.add('ales');
    if (hasTus) availableTypes.add('tus');
    
    if (availableTypes.isEmpty) {
      availableTypes.addAll(['tyt', 'ayt', 'ydt', 'yds', 'kpss_lisans', 'ekpss', 'dgs', 'ales', 'tus']);
    }

    if (_selectedExamType == null || !availableTypes.contains(_selectedExamType)) {
      _selectedExamType = availableTypes.first;
    }
                ...
                child: Row(
                  children: availableTypes.map((type) {
                    return Padding(
                      ...
                      child: _buildFilterChip(_getLabelForType(type), type),
                    );
                  }).toList(),
                ),
              ),
  ...
  String _getLabelForType(String type) {
    if (type == 'kpss_lisans') return 'KPSS Lisans';
    if (type == 'ekpss') return 'E-KPSS';
    if (type == 'ales') return 'ALES';
    if (type == 'tus') return 'TUS';
